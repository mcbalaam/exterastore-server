...
export const pluginsAllHandler = async ({ set }: Context) => {
	const plugins = await pluginService.getAllPlugins().catch(() => null);
	if (!plugins) {
		set.status = 404;
...
export const pluginsGetHandler = async ({ params, set }: Context) => {
	const plugin = await pluginService.getPluginById(params.id).catch(() => null);
	if (!plugin) {
		set.status = 404;
...
export const pluginsNamesHandler = async ({ set }: Context) => {
	const plugins = await pluginService.getAllPluginNames().catch(() => null);
	if (!plugins) {
		set.status = 404;
...
export const pluginsReleasesHandler = async ({ params, set }: Context) => {
	const plugins = await pluginService.getAllReleasesForPlugin(params.id).catch(() => null);
	if (!plugins) {
		set.status = 404;
...
export const pluginsReactionsHandler = async ({ params, set }: Context) => {
	const plugins = await pluginService.getReactions(params.id).catch(() => null);
	return plugins;
};

export const pluginsCreateHandler = async ({ body, set }: Context) => {
	const { name, license, description } = body as { name: string, license: string, description?: string };

	try {
		const result = await pluginService.createPlugin(name, license, description);
...
export const pluginsUpdateHandler = async ({ params, body, set }: Context) => {
	...
	try {
		const result = await pluginService.updatePlugin(params.id, name, description);
...
export const pluginsDeleteHandler = async ({ params, set }: Context) => {
	try {
		const result = await pluginService.deletePlugin(params.id);
		return result;
	} catch (error) {
		set.status = 404;
		return { error: error instanceof Error ? error.message : String(error) };
...
export const pluginsAddReleaseHandler = async ({ params, body, set }: Context) => {
	...
	try {
		const result = await pluginService.createRelease(params.id, fileReference, releaseHash, releaseNotes);
		return result;
	} catch (error) {
		set.status = 400;
		return { error: error instanceof Error ? error.message : String(error) };
...
export const pluginsStarHandler = async ({ params, body, set }: Context) => {
	...
	try {
		const result = await pluginService.addPluginStar(userId, params.id);
		return result;
	} catch (error) {
		set.status = 400;
		return { error: error instanceof Error ? error.message : String(error) };
